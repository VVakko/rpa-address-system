...
venv-deps-patch:  # Patch m3-gar module (don't show this menu in make main menu)
	...
	$(ACTIVATE) && patch `pip show m3-gar | grep 'Location' | sed -e 's/^Location: //'`/m3_gar/management/commands/manage_constraints.py -p0 <./misc/m3-gar-1.0.33/manage_constraints.patch
	$(ACTIVATE) && patch `pip show m3-rest-gar | grep 'Location' | sed -e 's/^Location: //'`/m3_rest_gar/filters.py -p0 <./misc/m3-rest-gar-1.0.37/filters.patch
